...
pub fn get_info() -> HashMap<String, String> {
    ...
    for p in str::from_utf8(&decoded).unwrap().split("|") { // paths are separated by the character "|" and then base64'd
       println!("Path searched: {}", p);
        if let Some(info) = get_files_from_glob_path(p) {
            if !info.eq("") { // remove empty results
